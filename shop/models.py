...
class Order(models.Model):
    ...
    user_id = models.ForeignKey(User,on_delete=models.SET_NULL,blank=True,null=True,help_text="Customer who owns the order")
    ...
    shipping_address  = models.CharField(max_length=255, help_text="Shipping address for the order" )
    payment_status = models.CharField(max_length=50, choices=PAYMENT_STATUS, default="UNPAID", help_text="Payment status of the order"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"Order {self.id} place by {self.user.username}"
...
class OrderItem(models.Model):
    ...
    def __str__(self):
        return f"{self.quantity} x {self.product_id.product_name} (Order {self.order_id})"
